...
def save_clustering_analysis_plot(
    ...
    tp: int, fp_dict: dict, fn: int, tn_dict: dict,
    ...
    if all_points_pos:
        ...
        padding = (max_lim - min_lim) * 0.1 # Add 10% padding
        ax1.set_xlim(min_lim[0]-padding[0], max_lim[0]+padding[0]); ax1.set_ylim(min_lim[1]-padding[1], max_lim[1]+padding[1]); ax1.set_zlim(min_lim[2]-padding[2], max_lim[2]+padding[2])
        ax2.set_xlim(min_lim[0]-padding[0], max_lim[0]+padding[0]); ax2.set_ylim(min_lim[1]-padding[1], max_lim[1]+padding[1]); ax2.set_zlim(min_lim[2]-padding[2], max_lim[2]+padding[2])
    ...
    bars = ax3.bar(count_labels, count_values, color=colors)
    ax3.bar_label(bars)
    ax3.set_title("Overall Detection Counts")
    ax3.set_ylabel("Number of Points")

    # === PLOT 4: FP/TN Breakdown (Bar Chart) ===
    ax4 = axes[1, 1]
    ...
    bar_width = 0.35
    index = np.arange(len(noise_labels))
    
    bars1 = ax4.bar(index - bar_width/2, fp_values, bar_width, label='False Pos (Clustered)', color='#F44336')
    bars2 = ax4.bar(index + bar_width/2, tn_values, bar_width, label='True Neg (Filtered)', color='#9E9E9E')
    
    ax4.bar_label(bars1)
    ax4.bar_label(bars2)
    ax4.set_title('Noise Filter Performance Breakdown')
